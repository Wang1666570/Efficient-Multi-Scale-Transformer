...
class PairLoader(Dataset):
	...
	def __getitem__(self, idx):
		...
		if self.mode == 'test':
			source_h, source_w, c = source_img.shape
			source_h_new = int(source_h / 16) * 16
			source_w_new = int(source_w / 16) * 16
			source_img = cv2.resize(source_img, dsize=(source_w_new, source_h_new), interpolation=cv2.INTER_AREA)

			target_h, target_w, c = target_img.shape
			target_h_new = int(target_h / 16) * 16
			target_w_new = int(target_w / 16) * 16
			target_img = cv2.resize(target_img, dsize=(target_w_new, target_h_new), interpolation=cv2.INTER_AREA)
